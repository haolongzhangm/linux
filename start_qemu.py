...
def main():
    ...
    cmd = "qemu-system-aarch64 -machine virt -cpu cortex-a72 -machine type=virt -smp %d -m %d -kernel arch/arm64/boot/Image --append \"root=/dev/vda rootfstype=ext4 rw exec init=init nokaslr console=ttyAMA0\" -hda %s -net nic,model=virtio -net user,hostfwd=tcp::2222-:22,hostfwd=tcp::9999-:9999" % (args.cpu_number, args.memory, args.system_image)
